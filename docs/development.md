...
| Reviewable, pinned PowerShell installer for a central Hermes observer and a local read-only fleet-status MCP | An installed fleet, telemetry transport, per-node agent runtime, remote node execution, autonomous recovery, or robot control |
