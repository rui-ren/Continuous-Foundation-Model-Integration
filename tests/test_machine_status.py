...
class MachineDoctorConfigurationTests(unittest.TestCase):
    def test_configuration_keeps_only_the_reviewed_read_only_surface(self) -> None:
        ...
        self.assertIn("--list-tool-names", content)
        self.assertIn("function Remove-HermesConfigIfPresent", content)
        self.assertIn("Config key not set:", content)
        self.assertIn("[Regex]::Escape($Key)", content)
